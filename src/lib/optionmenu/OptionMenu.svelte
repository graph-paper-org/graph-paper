<script>
  ...
  import { FloatingMenu, MenuList } from "../menu";
  import { onClickOutside } from "../core/utils";
  import { tooltip as tooltipAction } from "../core/actions/tooltip";
    ...
    <div class="option-menu__status-text">
      {#if multi}
        ...
        {#each range(0, $enabledOptions.length + 1) as i}
          <div
            style="visibility: {$selectedOptions.length === i
              ? 'visible'
              : 'hidden'};
      ...
      {:else}
        {#each $enabledOptions as opt, i}
          <div
            style="visibility: {opt.key === $selectedOptions[0].key
              ? 'visible'
              : 'hidden'};
