...
function toLocations(pt, xs, ys, left, right, top, bottom) {
  ...
  let locations = [...pt.map((p) => ({ ...p, xRange: Math.max(left, xs(p.x)), yRange: ys(p.y) }))];
  ...
  if (locations.length === 1) {
    locations[0].yRange = Math.min(bottom, Math.max(top, locations[0].yRange));
    return locations;
...
let locations = toLocations(point, $xScale, $yScale, $leftPlot, $rightPlot, $topPlot, $bottomPlot);
$: locations = toLocations(point, $xScale, $yScale, $leftPlot, $rightPlot, $topPlot, $bottomPlot);
let container;
let labelWidth = 0;
...
<g bind:this={container}>
  ...
  <Springable
    value={{
      y: (location.yRange || 0),
      x: (location.xRange + xBuffer + xOffset + (alignLabels ? labelWidth : 0)),
    }}
    let:springValue={v}
  ...
  {#if showPoints}
    {#each locations as {x, y, color, label}, i (label)}
      <Point scaling={false} x={x} y={y} color={color} size={3} />
    {/each}
  {/if}
</g>
