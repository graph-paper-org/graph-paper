<script>
  ...
  function addCancelable() {
    const country =
      countries[Math.round((countries.length - 1) * Math.random())];
...
<div style="width: var(--space-72x);">
  <Stack space={3}>
    ...
    <div>
      ...
      <ChipSet>
        <Chip toggled={which === 0} on:click={setToggle(0)}>
          <LineChart size={12} />
          Charts
        </Chip>
        <Chip toggled={which === 1} on:click={setToggle(1)}>
          <Table size={12} />
          Tables
        </Chip>
        <Chip toggled={which === 2} on:click={setToggle(2)}>
          <Information size={12} />
          Details
        </Chip>
        <Chip toggled={which === 3} on:click={setToggle(3)}>
          <Warning size={12} />
          Warnings
        </Chip>
      </ChipSet>
    ...
    <div>
      ...
      <ChipSet>
        <Chip filterable toggled={which === 0} on:click={setToggle(0)}>
          Charts
        </Chip>
        <Chip filterable toggled={which === 1} on:click={setToggle(1)}>
          Tables
        </Chip>
        <Chip filterable toggled={which === 2} on:click={setToggle(2)}>
          Details
        </Chip>
        <Chip filterable toggled={which === 3} on:click={setToggle(3)}>
          Warnings
        </Chip>
      </ChipSet>
    </div>
    ...
    <div>
      ...
      <ChipSet>
        <Chip
          filterable
          toggled={multipleFilters.includes(0)}
          on:click={handleMultipleFilter(0)}>
          US
        </Chip>
        <Chip
          filterable
          toggled={multipleFilters.includes(1)}
          on:click={handleMultipleFilter(1)}>
          GB
        </Chip>
        <Chip
          filterable
          toggled={multipleFilters.includes(3)}
          on:click={handleMultipleFilter(3)}>
          DE
        </Chip>
        <Chip
          filterable
          toggled={multipleFilters.includes(2)}
          on:click={handleMultipleFilter(2)}>
          BR
        </Chip>
      </ChipSet>
    </div>
      ...
      <ChipSet>
        {#each cancelables as { index, country }, i (index)}
          <div animate:flip={{ duration: 200 }}>
            <Chip cancelable on:cancel={removeCancelable(index)}>
              {country}
            </Chip>
          </div>
        {:else}
          <div
            style="font-size: var(--text-02);"
            transition:fade={{ duration: 10000, delay: 1000 }}>
            No Chips.
          </div>
        {/each}
      </ChipSet>
